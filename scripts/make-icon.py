...
def main() -> None:
    # Built as an alpha mask and then filled, so the mark is one colour with the
    # face punched out of it rather than dark marks painted on top. Monotone all
    # the way down: the .icns carries the same single fill the sidebar's SVG
    # does, and the holes show whatever is behind the icon.
    ...
    for sx in (x - int(ew * 0.55), x + w - int(ew * 0.45)):
        d.rounded_rectangle([sx, y + int(w * 0.34), sx + ew, y + int(w * 0.34) + eh],
                            radius=ew // 2, fill=255)

    d.rounded_rectangle([x, y, x + w, y + w], radius=int(w * 0.30), fill=255)

    # The face, cut back out of it.
    r = int(w * 0.095)
    ey = y + int(w * 0.44)
    for side in (-1, 1):
        cx = x + w // 2 + side * int(w * 0.175)
        d.ellipse([cx - r, ey - r, cx + r, ey + r], fill=0)
    ...
    d.arc([cx - sw, sy - sw // 2, cx + sw, sy + int(sw * 1.5)],
          start=25, end=155, fill=0, width=int(w * 0.058))
